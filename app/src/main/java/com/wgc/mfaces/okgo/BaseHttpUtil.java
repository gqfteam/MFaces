package com.wgc.mfaces.okgo;

import android.text.TextUtils;
import com.lzy.okgo.OkGo;
import com.lzy.okgo.callback.StringCallback;
import com.wgc.mfaces.util.LogUtil;
			...
			OkGo.get(url).tag(this).execute(new StringCallback() {
				...
				public void onSuccess(String t, Call call, Response response) {
					try {
                        ...
                        String resultCode = jsObj.optString("code");
                        if(resultCode.equals("0")) {
                            String data = jsObj.optString("data");
                            mCallBack.onSuccess(data);
                        } else {
                            String errMsg = jsObj.optString("error");
                            mCallBack.onFailure(-1, errMsg);
                        }
                    } catch (JSONException e) {
                        mCallBack.onFailure(-103, "JSON转换异常");
                        e.printStackTrace();
                    } catch (Exception e) {
                        mCallBack.onFailure(-105, "未知异常");
                        e.printStackTrace();
                    }
				}
